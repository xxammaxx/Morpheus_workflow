...
return [{json:{valid:!errors.length,errors,envelope:e,command,role,payload:p||{},github_owner:parts[0]||'',github_repo:parts[1]||'',issue_url:parts.length===2&&issue?`https://api.github.com/repos/${parts[0]}/${parts[1]}/issues/${issue}`:'',route:command,audit_row:{timestamp:new Date().toISOString(),actor:'control-tower',role,command,target:JSON.stringify(e.target||{}),project_id:p&&p.project_id||'',run_id:p&&p.run_id||'',result:errors.length?'REJECTED':'ACCEPTED',correlation_id:String(e.correlation_id||'')}}}];""", P(1, 0)))
    ...
    if cfg.cr_ssh:
        ...
        system_input = "const n=$('Read n8n System Status').first().json.data||$('Read n8n System Status').first().json,a=$('Read Adapter Runtime').first().json.data||$('Read Adapter Runtime').first().json,m=String($json.stdout||$json.data||''),n8nOk=Array.isArray(n)||Array.isArray(n.data),adapterOk=Boolean(a)&&a.free_first_enabled===true&&a.automatic_paid_agent_escalation===false,routerOk=adapterOk&&Array.isArray(a.providers)&&!a.providers.some(p=>/deepseek/i.test(String(p.provider||'')+' '+String(p.model||''))),mcpOk=Boolean(m)&&!/No MCP servers configured|permission denied|error/i.test(m),modules={n8n:n8nOk?'OK':'NICHT_OK',adapter:adapterOk?'OK':'NICHT_OK',router:routerOk?'OK':'NICHT_OK',mcp:mcpOk?'OK':'NICHT_OK'},ok=Object.values(modules).every(v=>v==='OK');return[{json:{status:ok?'OK':'NICHT_OK',overall:ok?'OK':'NICHT_OK',modules,source:'n8n-diagnostic-workflow',details:{n8n:n8nOk?'workflow API reachable':'workflow API unavailable',adapter:adapterOk?'runtime reachable':'runtime policy or adapter unavailable',router:routerOk?'free-first policy valid':'router policy invalid or DeepSeek present',mcp:mcpOk?'configured MCP discovery passed':'MCP discovery failed or no server configured'},mcp_output:m.slice(0,4000)}}];"
    else:
        system_input = "const n=$('Read n8n System Status').first().json.data||$('Read n8n System Status').first().json,a=$json.data||$json,n8nOk=Array.isArray(n)||Array.isArray(n.data),adapterOk=Boolean(a)&&a.free_first_enabled===true&&a.automatic_paid_agent_escalation===false,routerOk=adapterOk&&Array.isArray(a.providers)&&!a.providers.some(p=>/deepseek/i.test(String(p.provider||'')+' '+String(p.model||''))),modules={n8n:n8nOk?'OK':'NICHT_OK',adapter:adapterOk?'OK':'NICHT_OK',router:routerOk?'OK':'NICHT_OK',mcp:'NICHT_OK'},ok=false;return[{json:{status:'NICHT_OK',overall:'NICHT_OK',modules,source:'n8n-diagnostic-workflow',details:{n8n:n8nOk?'workflow API reachable':'workflow API unavailable',adapter:adapterOk?'runtime reachable':'runtime policy or adapter unavailable',router:routerOk?'free-first policy valid':'router policy invalid or DeepSeek present',mcp:'SSH diagnostic unavailable'}}}];"
    ...
    wf.add("Is System Test?", "Is Repo Analysis?", 1); wf.add("Is Repo Analysis?", "Fetch Repository Issues", 0); wf.add("Fetch Repository Issues", "Pack Repository Issues"); wf.add("Pack Repository Issues", "Run Project Analysis"); wf.add_node(respond_node("Respond Repo Analysis", P(9,5))); wf.add("Run Project Analysis", "Respond Repo Analysis")
    ...
    wf.add_node(code_node("Unsupported Valid Command", "return [{json:{status:'error',code:'COMMAND_NOT_IMPLEMENTED',command:$json.command,message:'No canonical action is registered for this command'}}];", P(6,8))); wf.add_node(respond_node("Respond Unsupported", P(7,8))); wf.add("Is Blueprint Start?", "Is Project Resume?", 1); wf.add("Is Project Resume?", "Reassess Project", 0); wf.add("Reassess Project", "Project Resume Result"); wf.add("Project Resume Result", "Respond Project Resume"); wf.add("Is Project Resume?", "Unsupported Valid Command", 1); wf.add("Unsupported Valid Command", "Respond Unsupported")
    return wf
