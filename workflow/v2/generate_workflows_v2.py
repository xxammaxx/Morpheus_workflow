...
const adm=new Set(['RUN_ROUTER_TEST','RUN_MCP_TEST','RUN_SYSTEM_TEST','REFRESH_CATALOG','SYNC_CREDENTIALS','RESTART_ADAPTER','GLOBAL_PROVIDER_DISABLE','GLOBAL_PROVIDER_ENABLE','GLOBAL_MODEL_DISABLE','GLOBAL_MODEL_ENABLE','DEPLOY_RELEASE']);
...
const repo=String(p&&p.repository_url||''); let u=null, parts=[];
if(repo){try{u=new URL(repo); parts=u.pathname.split('/').filter(Boolean); if(u.protocol!=='https:'||u.hostname!=='github.com'||u.username||u.password||u.search||u.hash||parts.length!==2)errors.push('REPOSITORY_URL_INVALID')}catch(x){errors.push('REPOSITORY_URL_INVALID')}}
if(['START_ISSUE','START_REPO_ANALYSIS','START_BLUEPRINT_PROJECT'].includes(command)&&!repo)errors.push('REPOSITORY_REQUIRED');
...
if(/deepseek/i.test(String(p&&p.model||'')))errors.push('DEEPSEEK_RETIRED');
...
return [{json:{valid:!errors.length,errors,envelope:e,command,role,payload:p||{},github_owner:parts[0]||'',github_repo:parts[1]||'',issue_url:parts.length===2&&issue?`https://api.github.com/repos/${parts[0]}/${parts[1]}/issues/${issue}`:'',route:command,audit_row:{timestamp:new Date().toISOString(),actor:'control-tower',role,command,target:JSON.stringify(e.target||{}),project_id:p&&p.project_id||'',run_id:p&&p.run_id||'',result:errors.length?'REJECTED':'ACCEPTED',correlation_id:String(e.correlation_id||'')}}}];""", P(1, 0)))
    ...
    wf.add_node(http_node("Run Project Analysis", "POST", cfg.webhook+"/webhook/autodev/project/analyse", "JSON.stringify({envelope: $('Restore Valid Command').first().json.envelope, issues: $json})", P(7,5), cfg.cr_api))
    wf.add("Is System Test?", "Is Repo Analysis?", 1); wf.add("Is Repo Analysis?", "Fetch Repository Issues", 0); wf.add("Fetch Repository Issues", "Run Project Analysis"); wf.add_node(respond_node("Respond Repo Analysis", P(8,5))); wf.add("Run Project Analysis", "Respond Repo Analysis")
    ...
    wf.add_node(code_node("Unsupported Valid Command", "return [{json:{status:'ACCEPTED',command:$json.command,message:'Command accepted at n8n boundary; canonical action is not active in this deployment'}}];", P(6,7))); wf.add_node(respond_node("Respond Unsupported", P(7,7))); wf.add("Is Blueprint Start?", "Unsupported Valid Command", 1); wf.add("Unsupported Valid Command", "Respond Unsupported")
    return wf
...
return[{json:{project_row:{project_id:projectId,name:p.project_name||`${e.github_owner}/${e.github_repo}`,repository_url:p.repository_url||'',blueprint_ref:p.blueprint_ref||'',project_mode:p.project_mode||'MANUAL',status:counts.BLOCKED?'BLOCKED':counts.RUNNING?'RUNNING':'READY',current_run_id:'',current_issue:'',created_at:new Date().toISOString(),updated_at:new Date().toISOString(),blueprint_sha256:p.blueprint_sha256||'',blueprint_coverage:'UNKNOWN'},issue_rows:rows,summary:{project_id:projectId,counts,source:'github-read-only'}}}];""", P(1,0)))
    wf.add_node(http_node("Persist Project Projection", "POST", cfg.project_rows(), "JSON.stringify({data: [$json.project_row], returnType: 'all'})", P(2,0), cfg.cr_n8n))
    wf.add_node(http_node("Persist Issue Projection", "POST", cfg.issue_rows(), "JSON.stringify({data: $('Normalize Project Analysis').first().json.issue_rows, returnType: 'all'})", P(3,0), cfg.cr_n8n))
    wf.add_node(code_node("Project Analysis Result", "const s=$('Normalize Project Analysis').first().json;return[{json:{status:'ACCEPTED',project:s.project_row,summary:s.summary,issue_count:s.issue_rows.length,source:'n8n-project-analysis'}}];", P(4,0)))
    wf.add_node(respond_node("Respond Project Analysis", P(5,0)))
    wf.add("Project Analysis Webhook","Normalize Project Analysis");wf.add("Normalize Project Analysis","Persist Project Projection");wf.add("Persist Project Projection","Persist Issue Projection");wf.add("Persist Issue Projection","Project Analysis Result");wf.add("Project Analysis Result","Respond Project Analysis")
    return wf
...
def build_07_blueprint_bootstrap(cfg):
    ...
    wf.add_node(http_node("Persist Blueprint Project","POST",cfg.project_rows(),"JSON.stringify({data: [$json.project_row], returnType: 'all'})",P(8,1),cfg.cr_n8n))
    ...
    wf.add_node(http_node("Persist Blueprint Issue Graph","POST",cfg.issue_rows(),"JSON.stringify({data: $('Restore Blueprint State').first().json.issue_rows, returnType: 'all'})",P(12,1),cfg.cr_n8n))
