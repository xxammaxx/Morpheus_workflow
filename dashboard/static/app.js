(() => {
  ...
  const commandApi = async (command, payload = {}, target = {}) => { const response = await fetch('/api/v1/commands', {method:'POST', headers:{'Content-Type':'application/json','X-Control-Tower-Token':token,'X-Control-Tower-Request':'1'}, body:JSON.stringify({command,payload,target})}); const value = await response.json(); if (!response.ok) throw Error(value.error || 'Command abgelehnt'); return value; };
  const jsonText = value => JSON.stringify(value ?? {}, null, 2);
  const renderProjects = projects => { const root = $('projects'); if (!root) return; if (!projects?.length) { root.innerHTML = '<p class="muted">Keine Projekte aus kanonischen Quellen verfügbar.</p>'; return; } root.innerHTML = projects.map(project => { const run = project.current_run ? `<button class="project-command" data-command="RESUME_RUN" data-run="${esc(project.current_run)}">Fortsetzen</button>` : '<button type="button" disabled title="Für dieses Projekt ist kein fortsetzbarer Run bekannt.">Fortsetzen · NOCH NICHT VERFÜGBAR</button>'; return `<article class="project-card"><h3>${esc(project.name)}</h3><p>${badge('Status', project.status)} ${badge('Fortschritt', `${project.progress.done} / ${project.progress.total}`)}</p><p><strong>Repository:</strong> ${esc(project.repository)}<br><strong>Aktuelles Issue:</strong> ${esc(project.current_issue || 'Nicht bekannt')}<br><strong>Run:</strong> ${esc(project.current_run || 'Nicht bekannt')}</p><div class="status-list">${Object.entries(project.issue_counts || {}).map(([key,value]) => badge(key, value)).join('')}</div><p>${run} <button data-view="projects">Backlog</button> <button data-view="new-work">Blueprint</button></p></article>`; }).join(''); };
  const renderDebug = data => { if (debugPaused) { pendingDebugData = data; return; } const root = $('event-stream'); if (!root) return; const events = data?.debugging?.events || []; const filter = $('debug-filter')?.value || 'ALL'; const currentOnly = $('debug-current-only')?.checked !== false; const current = data?.debugging?.current_run_id; const visible = events.filter(item => (!currentOnly || !current || item.run_id === current) && (filter === 'ALL' || (filter === 'ERROR' ? /fail|error|429|timeout/i.test(JSON.stringify(item)) : String(item.source || '').toLowerCase() === filter.toLowerCase()))); $('debug-flow').textContent = data?.debugging?.source === 'LIVE' ? `LIVE · ${current || 'Run nicht bekannt'} · Stage: ${data.debugging.stage || 'Nicht bekannt'}` : 'IDLE · Keine aktuelle Aktivität aus den autoritativen Quellen'; root.innerHTML = visible.length ? visible.map((item,index) => `<article class="event-card ${/fail|error|429|timeout/i.test(JSON.stringify(item)) ? 'event-error' : ''}" data-event-index="${index}"><span class="event-source">${esc(item.source || 'Unbekannte Quelle')}</span> → ${esc(item.target || 'Unbekanntes Ziel')} · ${esc(formatTimestamp(item.timestamp))}<br><strong>${esc(event(item.event || item.type || 'EVENT'))}</strong> · Contract: <code>${esc(item.contract || 'UNKNOWN')}</code><br><small>Run: ${esc(item.run_id || 'Nicht bekannt')} · Attempt: ${esc(item.attempt_id || 'Nicht bekannt')} · Correlation: ${esc(item.correlation_id || 'UNKNOWN')} · Modell: ${esc(item.model || 'Automatisch')}</small></article>`).join('') : '<p class="muted">Keine Events für den gewählten Filter.</p>'; visible.forEach((item,index) => root.querySelector(`[data-event-index="${index}"]`)?.addEventListener('click', () => { $('payload-inspector').hidden = false; $('payload-content').innerHTML = `<p>SOURCE: ${esc(item.source || 'UNKNOWN')}<br>TARGET: ${esc(item.target || 'UNKNOWN')}<br>TIMESTAMP: ${esc(formatTimestamp(item.timestamp))}<br>CONTRACT: ${esc(item.contract || 'UNKNOWN')}<br>RUN_ID: ${esc(item.run_id || 'UNKNOWN')}<br>ATTEMPT_ID: ${esc(item.attempt_id || 'UNKNOWN')}<br>VALIDATION: ${esc(item.validation || 'UNKNOWN')}</p><pre>${esc(jsonText(redactClient(item.payload === undefined ? {} : item.payload)))}</pre>`; })); };
  const redactClient = value => { if (!value || typeof value !== 'object') return value; if (Array.isArray(value)) return value.map(redactClient); const out = {}; Object.entries(value).forEach(([key,item]) => { if (/authorization|cookie|token|secret|password|api.?key|private.?key|reasoning|chain.?of.?thought/i.test(key)) out[key] = '[REDACTED]'; else if (key !== 'reasoning_content') out[key] = redactClient(item); }); return out; };
  const renderAdmin = data => { const router = ['Dynamischer Router','Modellkatalog','Free Pool','Credential-Erkennung','Capability Filter','Tool Routing','Vision Routing','Structured Output','Transport Failover','Semantic Failover','Run Blacklist','Paid Fallback Sperre','DeepSeek Sperre']; const mcp = (data?.runtime?.mcp_servers || data?.system_health?.mcp?.servers || []).map(item => typeof item === 'string' ? item : item.name).filter(Boolean); $('router-tests').innerHTML = router.map(name => `<p>${esc(name)} <button class="admin-action test-command" data-command="RUN_ROUTER_TEST" data-test="${esc(name)}">Testen</button> <span class="test-result muted">Noch nicht getestet</span></p>`).join(''); $('mcp-tests').innerHTML = mcp.length ? mcp.map(name => `<p>${esc(name)} <button class="admin-action test-command" data-command="RUN_MCP_TEST" data-test="${esc(name)}">Testen</button> <span class="test-result muted">Noch nicht getestet</span></p>`).join('') : '<p class="muted">Keine MCP-Server konfiguriert.</p>'; };
  const renderExtended = data => { renderProjects(data.projects || []); $('current-work').innerHTML = data.active_run ? `<p>${badge('Run', data.active_run.run_id)} ${badge('Status', state(data.active_run.state))}</p><p>Stage: ${esc(job(data.active_run.current_job || 'UNKNOWN'))}<br>Modell: automatisch über dynamischen Router</p>` : '<p class="muted">Keine aktuelle Aktivität.</p>'; $('delivery').innerHTML = `<p>Release: ${esc(data.release?.morpheus_release || 'Nicht bekannt')}</p><p>GitHub/Delivery: ${esc(data.sources?.n8n || 'UNAVAILABLE')}</p><p>Keine automatische Freigabe aus dem Dashboard.</p>`; const card = $('debug-card'); if (card) card.innerHTML = `<p>${badge(data.debugging?.source || 'IDLE', data.debugging?.source === 'LIVE' ? 'LIVE' : 'IDLE')}</p><p>Run: ${esc(data.debugging?.current_run_id || 'Nicht bekannt')} · Events: ${esc((data.debugging?.events || []).length)}</p><button data-view="debugging">Live öffnen</button>`; renderDebug(data); renderAdmin(data); };
  async function sendCommand(command, payload = {}, target = {}) { try { const value = await commandApi(command, payload, target); $('work-result') && ($('work-result').textContent = `Command ${command} an n8n übergeben · ${value.correlation_id || 'korreliert'}`); $('admin-result') && ($('admin-result').textContent = `Command ${command} auditierbar übergeben.`); return value; } catch (error) { $('work-result') && ($('work-result').textContent = error.message); $('admin-result') && ($('admin-result').textContent = error.message); return null; } }
  ...
  const renderDataEdges = data => { const root = $('data-edges'); if (!root) return; const observed = (data?.debugging?.events || []).filter(item => item.source && item.target && item.source !== 'UNKNOWN' && item.target !== 'UNKNOWN' && item.run_id && item.run_id !== 'UNKNOWN' && item.attempt_id && item.attempt_id !== 'UNKNOWN' && item.correlation_id && item.correlation_id !== 'UNKNOWN' && item.contract && item.contract !== 'UNKNOWN'); const edges = observed.map(item => ({source:item.source,target:item.target,contract:item.contract,event:item.event || item.type || 'EVENT',item})); root.innerHTML = edges.length ? edges.map((edge,index) => `<button class="edge-card" type="button" data-edge-index="${index}"><strong>${esc(edge.source)} → ${esc(edge.target)}</strong><br><code>${esc(edge.contract)}</code><br><small>Run: ${esc(edge.item.run_id)} · Attempt: ${esc(edge.item.attempt_id)} · Correlation: ${esc(edge.item.correlation_id)} · ${esc(edge.event)}</small></button>`).join('') : '<p class="muted">Keine LIVE-Kante aus korrelierten Ereignissen beobachtet.</p>'; root.querySelectorAll('[data-edge-index]').forEach(button => button.addEventListener('click', () => { const item = edges[Number(button.dataset.edgeIndex)]?.item || {}; $('payload-inspector').hidden = false; $('payload-content').innerHTML = `<p>SOURCE: ${esc(item.source || 'UNKNOWN')}<br>TARGET: ${esc(item.target || 'UNKNOWN')}<br>TIMESTAMP: ${esc(formatTimestamp(item.timestamp))}<br>CONTRACT: ${esc(item.contract || 'UNKNOWN')}<br>RUN_ID: ${esc(item.run_id || 'UNKNOWN')}<br>ATTEMPT_ID: ${esc(item.attempt_id || 'UNKNOWN')}<br>VALIDATION: ${esc(item.validation || 'UNKNOWN')}</p><pre>${esc(jsonText(redactClient(item.payload === undefined ? {} : item.payload)))}</pre>`; })); };
  const baseRender = render;
  render = async () => { if (rendering) return; await baseRender(); try { const data = await api('/api/v1/overview'); renderExtended(data); renderDataEdges(data); } catch (_) { /* base render owns auth/degraded UI */ } };
  document.addEventListener('click', eventObject => { const button = eventObject.target?.closest?.('[data-view]'); if (!button) return; show(button.dataset.view); if (button.dataset.view === 'system-map') renderDiagram('system'); if (button.dataset.view === 'data-flow') renderDiagram('data'); });
  $('debug-filter')?.addEventListener('change', () => latestOverview && renderDebug(latestOverview));
  $('debug-current-only')?.addEventListener('change', () => latestOverview && renderDebug(latestOverview));
  $('debug-pause')?.addEventListener('click', eventObject => { debugPaused = !debugPaused; eventObject.currentTarget.dataset.paused = String(debugPaused); eventObject.currentTarget.textContent = debugPaused ? 'Anzeige fortsetzen' : 'Anzeige pausieren'; if (!debugPaused) { pendingDebugData = null; render(); } });
  $('debug-now')?.addEventListener('click', () => render());
  $('payload-close')?.addEventListener('click', () => $('payload-inspector').hidden = true);
  $('new-work-form')?.addEventListener('submit', async eventObject => { eventObject.preventDefault(); const command = document.querySelector('input[name="work-type"]:checked')?.value; const payload = {repository_url:$('work-repository').value.trim() || undefined, issue:$('work-issue').value.trim() || undefined, additional_instruction:$('work-instruction').value.trim() || undefined, blueprint_md:$('work-blueprint').value || undefined, project_name:$('work-name').value.trim() || undefined, project_mode:$('work-mode').value}; Object.keys(payload).forEach(key => payload[key] === undefined && delete payload[key]); await sendCommand(command, payload); });
  const commandButtonSelector = '.test-command,.admin-action,.project-command';
  const runCommandButton = async button => {
    if (button.dataset.commandInFlight === 'true') return;
    button.dataset.commandInFlight = 'true';
    button.disabled = true;
    const result = button.classList.contains('test-command') ? button.nextElementSibling : $('admin-result');
    try {
      const value = button.classList.contains('project-command')
        ? await sendCommand(button.dataset.command, {run_id:button.dataset.run})
        : await commandApi(button.dataset.command, {test:button.dataset.test || null, read_only:true});
      if (result && value) { result.textContent = value.result?.status || value.status || 'ANGEFORDERT'; result.className = 'ok'; }
    } catch (error) {
      if (result) { result.textContent = error.message; result.className = 'error'; }
    } finally {
      button.disabled = false;
      delete button.dataset.commandInFlight;
    }
  };
  document.addEventListener('click', eventObject => { const button = eventObject.target?.closest?.(commandButtonSelector); if (!button || button.disabled) return; eventObject.preventDefault(); void runCommandButton(button); });
