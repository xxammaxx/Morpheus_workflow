(() => {
  ...
  const renderAdmin = () => { const router = ['Dynamischer Router','Modellkatalog','Free Pool','Credential-Erkennung','Capability Filter','Tool Routing','Vision Routing','Structured Output','Transport Failover','Semantic Failover','Run Blacklist','Paid Fallback Sperre','DeepSeek Sperre']; const mcp = ['GitHub MCP','Filesystem MCP','n8n MCP','Proxmox MCP']; $('router-tests').innerHTML = router.map(name => `<p>${esc(name)} <button class="admin-action test-command" data-command="RUN_ROUTER_TEST" data-test="${esc(name)}">Testen</button> <span class="test-result muted">Noch nicht getestet</span></p>`).join(''); $('mcp-tests').innerHTML = mcp.map(name => `<p>${esc(name)} <button class="admin-action test-command" data-command="RUN_MCP_TEST" data-test="${esc(name)}">Testen</button> <span class="test-result muted">Noch nicht getestet</span></p>`).join(''); document.querySelectorAll('.test-command,.admin-action').forEach(button => button.addEventListener('click', async () => { const result = button.nextElementSibling || $('admin-result'); if (button.classList.contains('admin-action') && button.dataset.command === 'RUN_SYSTEM_TEST') button.disabled = true; try { const value = await commandApi(button.dataset.command, {test:button.dataset.test || null, read_only:true}); if (result) { result.textContent = value.result?.status || value.status || 'ANGEFORDERT'; result.className = 'ok'; } } catch (error) { if (result) { result.textContent = error.message; result.className = 'error'; } } finally { button.disabled = false; } })); };
  ...
  const renderDataEdges = data => { const root = $('data-edges'); if (!root) return; const run = data?.active_run || {}; const edges = [['n8n','Adapter','autodev.plan.request.v1'],['Adapter','Normalizer','Harness response'],['Normalizer','Router','normalized request'],['Router','OpenCode','selected free model'],['OpenCode','Adapter','autodev.plan.v1'],['Adapter','n8n','Plan Gate']]; root.innerHTML = edges.map(([source,target,contract]) => `<button class="edge-card" type="button" data-edge="${esc(contract)}"><strong>${esc(source)} → ${esc(target)}</strong><br><code>${esc(contract)}</code><br><small>Run: ${esc(run.run_id || 'Nicht bekannt')} · Status: ${esc(run.state || 'IDLE')}</small></button>`).join(''); root.querySelectorAll('[data-edge]').forEach(edge => edge.addEventListener('click', () => { $('payload-inspector').hidden = false; $('payload-content').innerHTML = `<p>Quelle: ${esc(edge.textContent.split(' → ')[0])} · Ziel: Korrelationskante<br>Contract: ${esc(edge.dataset.edge)}<br>Validation: nur beobachtete Metadaten</p><pre>${esc(jsonText({run_id:run.run_id || null, contract:edge.dataset.edge, payload:'[REDACTED_UNLESS_OBSERVED]'}))}</pre>`; })); };
  const baseRender = render;
  render = async () => { await baseRender(); try { const data = await api('/api/v1/overview'); renderExtended(data); renderDataEdges(data); } catch (_) { /* base render owns auth/degraded UI */ } };
  ...
  $('debug-pause')?.addEventListener('click', eventObject => { eventObject.currentTarget.dataset.paused = eventObject.currentTarget.dataset.paused !== 'true'; eventObject.currentTarget.textContent = eventObject.currentTarget.dataset.paused === 'true' ? 'Anzeige fortsetzen' : 'Anzeige pausieren'; });
